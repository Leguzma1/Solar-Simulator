...
static void build_html(char *buf, size_t buf_len)
{
    ...
    if (strcmp(white_led.state_str, "off") == 0) {
...
{
    led_t *led = (led_t *) req->user_ctx;
    led->state_str = "on";
    led->power = LED_FLAG_ON;
    led_power_control(led, led->power);
    // gpio_set_level(led->gpio, led->state_flag);
